...
SELECT
    ...
    `shared_filesystem`.`network_mode` AS `network_mode`,
    `shared_filesystem`.`ip_address` AS `ip_address`,
    `shared_filesystem`.`cidr` AS `cidr`,
    `shared_filesystem`.`gateway` AS `gateway`,
    `shared_filesystem`.`dns1` AS `dns1`,
    `shared_filesystem`.`dns2` AS `dns2`,
