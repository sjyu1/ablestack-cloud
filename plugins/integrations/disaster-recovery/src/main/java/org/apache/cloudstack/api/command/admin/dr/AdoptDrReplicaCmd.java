...
@APICommand(name = AdoptDrReplicaCmd.APINAME, description = "Adopt a DR replica", responseObject = DrRunResponse.class,
        responseView = ResponseObject.ResponseView.Full, authorized = {RoleType.Admin})
public class AdoptDrReplicaCmd extends AbstractDrPlanActionCmd {
    public static final String APINAME = "adoptDrReplica";

    @Parameter(name = "cleanuptransport", type = CommandType.BOOLEAN,
            description = "whether to clean up the temporary DR transport after adopting the replica")
    ...
    protected void addRequestProperties(JsonObject request) {
        addProperty(request, "cleanupTransport", cleanupTransport);
    }
    ...
    public String getEventDescription() {
        return "Adopting DR replica for plan " + getPlanId();
    }
}
