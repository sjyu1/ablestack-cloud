...
openssl enc -aes-256-cbc -d -K $key -pass pass:$kek_pass -saltlen 16 -md sha256 -iter 100000 -in /etc/cloudstack/management/db.properties.enc -out /etc/cloudstack/management/db.properties > /dev/null 2>&1
...
if [ ! "$check" -eq 0 ]; then
        ...
        if [ ! -n "$failed_files" ]; then
                mysql --user=root --password=$database_password -e "use cloud; INSERT INTO security_check (mshost_id, check_result, check_date, check_failed_list, type) VALUES ('1', '1', DATE_SUB(NOW(), INTERVAL 9 HOUR), '$failed_files', 'Initial')"  > /dev/null 2>&1
                UUID=$(cat /proc/sys/kernel/random/uuid)
                mysql --user=root --password=$database_password -e "use cloud; INSERT INTO event (uuid, type, state, description, user_id, account_id, domain_id, resource_id, created, level, start_id, archived, display, client_ip) VALUES ('$uuid', 'SECURITY.CHECK', 'Completed', 'Successfully completed security check on the management server when running the product.', '2', '2', '1', '0', DATE_SUB(NOW(), INTERVAL 9 HOUR), 'INFO', '0', '0', '1', '$host_ip');" > /dev/null 2>&1
        else
                mysql --user=root --password=$database_password -e "use cloud; INSERT INTO security_check (mshost_id, check_result, check_date, check_failed_list, type) VALUES ('1', '0', DATE_SUB(NOW(), INTERVAL 9 HOUR), '$failed_files', 'Initial')"  > /dev/null 2>&1
                UUID=$(cat /proc/sys/kernel/random/uuid)
                mysql --user=root --password=$database_password -e "use cloud; INSERT INTO alert (uuid, type, pod_id, data_center_id, subject, sent_count, created, last_sent, archived, name) VALUES ('$uuid', '14', '0', '0', 'Management server node security check failed when running the product.', '1', DATE_SUB(NOW(), INTERVAL 9 HOUR), DATE_SUB(NOW(), INTERVAL 9 HOUR), '0', 'ALERT.MANAGEMENT');" > /dev/null 2>&1
                UUID=$(cat /proc/sys/kernel/random/uuid)
                mysql --user=root --password=$database_password -e "use cloud; INSERT INTO event (uuid, type, state, description, user_id, account_id, domain_id, resource_id, created, level, start_id, archived, display, client_ip) VALUES ('$uuid', 'SECURITY.CHECK', 'Completed', 'Failed to execute security check on the management server when running the product.', '2', '2', '1', '0', DATE_SUB(NOW(), INTERVAL 9 HOUR), 'INFO', '0', '0', '1', '$host_ip');" > /dev/null 2>&1
        fi
        ...
        echo "db.properties decrypt failed."
        # override variable to 0 and 1
        for var in {1..5}
        do 
                key=01010101
