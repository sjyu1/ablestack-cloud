...
if [ $# -gt 0 ] ; then
    ...
    if [ -e "$File" ]; then
        count1=$(systemctl status mold-monitoring.service | grep -i running | wc -l)
        if [ "$count1" -eq 0 ]; then
