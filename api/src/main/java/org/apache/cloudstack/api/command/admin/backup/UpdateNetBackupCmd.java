...
@APICommand(name = "updateNetBackup",
        description = "Update NetBackup backup metadata on successful NetBackup completion",
        responseObject = SuccessResponse.class, since = "4.22.0",
        authorized = {RoleType.Admin})
public class UpdateNetBackupCmd extends BaseCmd {
    ...
    public void execute() {
        try {
            boolean result = backupManager.updateNetBackup(this);
            if (result) {
                SuccessResponse response = new SuccessResponse(getCommandName());
                response.setResponseName(getCommandName());
                setResponseObject(response);
            } else {
                throw new CloudRuntimeException("Error while updating NetBackup metadata for Instance backup");
            }
        } catch (Exception e) {
