...
@APICommand(name = "listStorageSmbAcls",
        responseObject = StorageAccessRuleResponse.class,
        description = "Lists SMB ACL rules.",
        requestHasSensitiveInfo = false,
        responseHasSensitiveInfo = false,
        since = "4.21.0",
        authorized = {RoleType.Admin, RoleType.ResourceAdmin, RoleType.DomainAdmin, RoleType.User})
public class ListStorageSmbAclsCmd extends BaseListCmd implements UserCmd {
    @Inject
    StorageService storageService;

    @Parameter(name = ApiConstants.ID, type = CommandType.UUID, entityType = StorageAccessRuleResponse.class, description = "ACL ID")
    private Long id;
    ...
    @Override
    public void execute() {
        ListResponse<StorageAccessRuleResponse> response = storageService.listStorageSmbAcls(this);
